        ...
        data={
            "summary": {
                ...
                "informationRatio": information_ratio(
                    nav_series, service._ppy(market_id), service._config.annual_risk_free_rate
                ),
            },
...
async def get_analysis_trading(
    ...
    monthly = await service.get_monthly_trade_stats(market_id, account_id, strategy_id, start_time, end_time)
    ...
    return ApiResponse(
        data={
            ...
            "monthlyTrades": [
                {"month": m, "count": c, "winRate": round(w * 100, 1)}
                for m, c, w in monthly
            ],
        }
    )
...
async def get_analysis_strategy(
    ...
    for performance in comparison.performances:
        ...
        if comparison.correlation is not None and performance.strategy_id in comparison.correlation.labels:
            ...
            correlation = [
                {"strategy": label, "value": comparison.correlation.matrix[row_index][index]}
